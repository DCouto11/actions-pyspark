...
df_prod_clean = Func.clean_dataframe(df_products)
df_sales_clean = Func.clean_dataframe(df_sales)
df_stores_clean = Func.clean_dataframe(df_stores)

# DataFrame transformations (sales aggregation, monthly sales insights)
df_agg_sales, df_monthly_sales, df_enriched_sales = Func.transform_dataframe(df_prod_clean, df_sales_clean, df_stores_clean)
